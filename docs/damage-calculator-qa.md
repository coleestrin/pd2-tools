...
- Martial Arts charge profiles include only the selected stage's standalone attack payload. Earlier stages, other charge skills, and kick-finisher releases are excluded.
...
- Detected item/mercenary auras, Call to Arms Battle Command, and allocated castable buffs initialize the same removable selection rows used for manual choices; base profiles contain no hidden aura contribution. Same-name sources use the highest automatic level, and a user-selected level overrides it only when higher.
